...
export function bootMelonPlatform(): Promise<MelonPlatform> {
  ...
  booted = (async () => {
    // Prefer IndexedDB persistence, but a lot of locked-down environments
    // (private browsing, some school Chromebooks) block it. If that happens we
    // fall back to an in-memory filesystem so the app still works for the
    // session rather than hanging or white-screening.
    let sb: any;
    try {
      sb = await createSubstrate({ persist: true, dirs: DIRS });
    } catch (e) {
      console.warn('[MelonSynth] persistent storage unavailable, using in-memory FS:', e);
      sb = await createSubstrate({ persist: false, dirs: DIRS });
      (window as any).__melonEphemeral = true;
    }
